...
class Pokemon(pygame.sprite.Sprite):
    ...
    def __init__(self, name, level, type, hp, attack, defense, speed, moves, x, y, side="front"):
        ...
        #Besiegte Pokemon als solche markieren
    ...
    def take_damage(self, damage):
        ...
        if self.current_hp < 0:
            self.current_hp = 0
        if self.current_hp == 0:
            self.is_fainted = True  #Pokemon als besiegt markieren    
...
def switch_pokemon(team, current_pokemon):
    ...
    current_index = team.index(current_pokemon)
    # Gehe alle Pokémon nach dem aktuellen durch
    for i in range(1, len(team)):
        next_index = (current_index + i) % len(team)
        next_pokemon = team[next_index]
        if not next_pokemon.is_fainted:
            return next_pokemon
    # Kein anderes Pokémon verfügbar
    return None
...
def initialize_teams():
    ...
    for pkm in player_team + rival_team:
        pkm.reset()  # Setzt auch is_fainted zurück
...
while game_status != 'quit': #Läuft bis der Spieler das Spiel beendet
    ...
    if game_status == 'switch':
        ...
        # Nach der Auswahl direkt zum nächsten Status wechseln
        continue
    ...
    if game_status == 'fainted':
        ...
        if rival_pokemon.current_hp == 0:
            rival_pokemon.is_fainted = True
            #nächstes besiegtes Pokemon suchen
            next_rival = next((pkm for pkm in rival_team if not pkm.is_fainted), None)
            if next_rival:  # Prüfen, ob noch Pokémon übrig sind
                rival_pokemon = next_rival  # Nächstes Pokémon wählen
        ...
        # Wenn das Spieler-Pokémon besiegt wurde, zeige das Switch-Menü
